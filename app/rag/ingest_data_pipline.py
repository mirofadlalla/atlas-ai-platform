...
class RAGPipeline:
    ...
    def process_file(file_path: str, custom_metadata: dict, db: Session):
        ...
        # 3. Load the file and get a list of Document objects
        documents = DocumentLoader.load_file(file_path, custom_metadata)

        # 4. Extract text and ingest into Qdrant
        full_text = "\n\n".join([doc.page_content for doc in documents])
        result = main(full_text, custom_metadata)
        
        # 5. Mark the file as processed with its hash to avoid reprocessing unchanged files in the future
        FileTracker.mark_file_as_processed(tenant_id, file_name, file_hash,db)
